...
local TimeFmt = QuartzE.Util.TimeFormat
...
local function ToggleCastNotInterruptible(self, notInterruptible, init)
	...
	if notInterruptible and db.noInterruptChangeBorder then
		...
		r,g,b = unpack(QuartzE.db.profile.bordercolor)
		a = QuartzE.db.profile.borderalpha
	end
	self:SetBackdrop(self.backdrop)
	self:SetBackdropBorderColor(r, g, b, a)

	r, g, b = unpack(QuartzE.db.profile.backgroundcolor)
	self:SetBackdropColor(r, g, b, QuartzE.db.profile.backgroundalpha)
...
function CastBarTemplate:UNIT_SPELLCAST_START(event, unit, guid, spellID)
	...
	self.Bar:SetStatusBarColor(unpack(self.casting and QuartzE.db.profile.castingcolor or QuartzE.db.profile.channelingcolor))
	...
	if icon == "Interface\\Icons\\Temp" and QuartzE.db.profile.hidesamwise then
		icon = nil
	end
...
function CastBarTemplate:UNIT_SPELLCAST_STOP(event, unit)
	...
	self.Bar:SetStatusBarColor(unpack(QuartzE.db.profile.completecolor))
...
function CastBarTemplate:UNIT_SPELLCAST_FAILED(event, unit)
	...
	self.Bar:SetStatusBarColor(unpack(QuartzE.db.profile.failcolor))
...
function CastBarTemplate:ApplySettings()
	...
	self.TimeText:SetTextColor(unpack(QuartzE.db.profile.timetextcolor))
	...
	self.Text:SetTextColor(unpack(QuartzE.db.profile.spelltextcolor))
	...
	self.Spark:SetVertexColor(unpack(QuartzE.db.profile.sparkcolor))
...
do
	local function getBar(info)
		return QuartzE.CastBarTemplate.bars[info[1]]
	end
	...
	local function copySettings(info, v)
		local bar = getBar(info)
		local from = QuartzE:GetModule(v)
		QuartzE:CopySettings(from.db.profile, bar.config)
		bar:ApplySettings()
	end

	local function getEnabled(info)
		local bar = getBar(info)
		return QuartzE:GetModuleEnabled(bar.modName)
	end

	local function setEnabled(info, v)
		local bar = getBar(info)
		return QuartzE:SetModuleEnabled(bar.modName, v)
	end
...
QuartzE.CastBarTemplate = {}
QuartzE.CastBarTemplate.defaults = {
...
QuartzE.CastBarTemplate.template = CastBarTemplate
QuartzE.CastBarTemplate.bars = {}
function QuartzE.CastBarTemplate:new(parent, unit, name, localizedName, config)
	...
	QuartzE.CastBarTemplate.bars[name] = bar
	...
	bar.Bar      = QuartzE:CreateStatusBar(nil, bar) --CreateFrame("StatusBar", nil, bar)
