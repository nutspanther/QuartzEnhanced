...
local QuartzE = LibStub("AceAddon-3.0"):NewAddon("QuartzE", "AceConsole-3.0")
...
function QuartzE:OnEnable()
	...
	CONFIGMODE_CALLBACKS["QuartzE"] = function(action)
		if action == "ON" then
			self:Unlock(false)
...
function QuartzE:ApplySettings()
	db = self.db.profile
...
function QuartzE:ToggleLock(showUI)
	local func = self.unlock and "Lock" or "Unlock"
	self[func](self, showUI)
end

function QuartzE:Unlock(showUI)
	self.unlock = true
	for k,v in self:IterateModules() do
...
function QuartzE:ShowUnlockDialog()
	if not self.unlock_dialog then
		...
		lockBars:SetScript("OnClick", function(self)
			QuartzE:Lock()
			LibStub("AceConfigRegistry-3.0"):NotifyChange("QuartzE")
		end)
...
function QuartzE:CopySettings(from, to)
...
function QuartzE:GetModuleEnabled(module)
	return db.modules[module]
end

function QuartzE:SetModuleEnabled(module, value)
...
function QuartzE:Merge(source, target)
...
QuartzE.Util = {}
function QuartzE.Util.TimeFormat(num, isCastTime)
