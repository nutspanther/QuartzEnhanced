...
local Mirror = QuartzE:NewModule(MODNAME, "AceHook-3.0", "AceEvent-3.0", "AceTimer-3.0")
local Player = QuartzE:GetModule("Player")
local Focus = QuartzE:GetModule("Focus")
local Target = QuartzE:GetModule("Target")
...
local mirrorbars = setmetatable({}, {
	__index = function(t,k)
		...
		local bar = QuartzE:CreateStatusBar("QuartzMirrorBar" .. tostring(k), UIParent)
		t[k] = bar
...
function Mirror:OnInitialize()
	...
	QuartzE:RegisterModuleOptions(MODNAME, getOptions, L["Mirror"])
