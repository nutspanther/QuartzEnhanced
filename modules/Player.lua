...
local Player = QuartzE:NewModule(MODNAME)
...
local defaults = {
	profile = QuartzE:Merge(QuartzE.CastBarTemplate.defaults,
	{
		hideblizz = true,
		showticks = true,
...
function Player:OnInitialize()
	...
	QuartzE:RegisterModuleOptions(MODNAME, getOptions, L["Player"])

	self.Bar = QuartzE.CastBarTemplate:new(self, "player", MODNAME, L["Player"], db)
...
function Player:OnHide()
	local Latency = QuartzE:GetModule(L["Latency"],true)
	if Latency then
...
local sparkfactory = {
	__index = function(t,k)
		...
		spark:SetVertexColor(unpack(QuartzE.db.profile.sparkcolor))
