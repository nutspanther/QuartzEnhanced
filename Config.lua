...
do
	function getOpt(info)
		local key = info[#info] 
		return QuartzE.db.profile[key]
	end
	
	function setOpt(info, value)
		local key = info[#info]
		QuartzE.db.profile[key] = value
		QuartzE:ApplySettings()
	end
...
function QuartzE:ChatCommand(input)
	if not input or input:trim() == "" then
		InterfaceOptionsFrame_OpenToCategory(QuartzE.optFrames.Profiles)
		InterfaceOptionsFrame_OpenToCategory(QuartzE.optFrames.QuartzE)
	else
		LibStub("AceConfigCmd-3.0").HandleCommand(QuartzE, "quartz", "QuartzE", input)
	end
end

function QuartzE:SetupOptions()
	self.optFrames = {}
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable("QuartzE", getOptions)
	self.optFrames.QuartzE = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("QuartzE", "Quartz Enhanced", nil, "general")
	self:RegisterModuleOptions("Profiles", LibStub("AceDBOptions-3.0"):GetOptionsTable(self.db), "Profiles")
	local LibDualSpec = LibStub('LibDualSpec-1.0')
	LibDualSpec:EnhanceDatabase(self.db, "QuartzE")
	...
	self:RegisterChatCommand("qe", "ChatCommand")
end

function QuartzE:RegisterModuleOptions(name, optTable, displayName)
	moduleOptions[name] = optTable
	self.optFrames[name] = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("QuartzE", displayName or name, "Quartz Enhanced", name)
end
